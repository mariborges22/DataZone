...
from geoalchemy2.functions import ST_AsGeoJSON, ST_Simplify, ST_MakeEnvelope, ST_Intersects
    ...
    try:
        ...
        if simplify:
            query = query.add_columns(
                ST_AsGeoJSON(ST_Simplify(Subestacao.geometry, settings.SIMPLIFY_TOLERANCE)).label("geometry")
            )
        else:
            query = query.add_columns(
        ...
        if bbox:
            ...
            try:
                ...
                raise HTTPException(status_code=400, detail="Formato de bbox inválido. Use: min_lon,min_lat,max_lon,max_lat")
