...
def validate_gdf(gdf: gpd.GeoDataFrame, layer_name: str) -> gpd.GeoDataFrame:
    ...
    if invalid_geoms.any():
        logger.warning(f"Encontradas {invalid_geoms.sum()} geometrias inválidas. Tentando corrigir...")
    ...
    final_count = len(gdf)
    logger.info(f"Registros após validação: {final_count} ({original_count - final_count} removidos)")
...
def process_subestacoes(gdb_path: str, layer_name: str = None) -> None:
    ...
    try:
        ...
        if layer_name is None:
            ...
            possible_names = ['subestacao', 'subestacoes', 'substation', 'substations', 'se']
            layer_name = next(
